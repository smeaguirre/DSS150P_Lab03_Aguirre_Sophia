...
def validate_curated(df: pd.DataFrame) -> list[str]:
    ...
    # 4. allowed statuses
    allowed_statuses = {'PENDING', 'SHIPPED', 'DELIVERED', 'CANCELLED', 'PACKED', 'PAID'}

    # Find any statuses in the dataframe that aren't in our allowed list
    invalid_statuses = df[~df['status'].isin(allowed_statuses)]['status'].unique()

    if len(invalid_statuses) > 0:
        errors.append(f"Validation Error: Unrecognized 'status' values found: {invalid_statuses}")
